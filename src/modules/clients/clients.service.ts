...
export class ClientsService {
  ...
  constructor(
    ...
    this.axiosInstance = axios.create({
        ...
        headers: {
            Authorization: `${configService.get<string>("b54_API_KEY")}`,
            "Content-Type": 'application/json'
        }
    })
    this.customerPartnerId = configService.get<string>("b54_CUSTOMER_PARTNER_ID")
  }

  async create(createClientDto: CreateClientDto) {
    try {
      let result = await this.axiosInstance.get(`customer_partners/${this.customerPartnerId}/customers`)
